...
static uint32_t s_measurement_id = 0; // increases by one each time a new measurement is done
                          ...
                          int32_t event_id, void *event_data)
{
	if (event_id == WIFI_EVENT_STA_CONNECTED) {
        ...
        memcpy(s_peer_mac, event->peer_mac, 6);
...
static void ftm_print_report(uint32_t id)
{
    int i;
    char *log = NULL;
    ...
    for (i = 0; i < s_ftm_report_num_entries; i++) {
        ...
        bzero(log, 200);
        log_ptr += sprintf(log_ptr, "%" PRIu32 ", ", id);
...
static bool start_ap(const char* ssid, const char* pass, uint8_t channel, uint8_t bw) {
    s_reconnect = false;
...
static int wifi_cmd_ftm(int argc, char **argv)
{
    ...
    if (bits & CONNECTED_BIT) {
        ...
        ap_record = find_ftm_responder_ap(AP_SSID);
        if (ap_record) {
...
#if defined(CONFIG_IS_AP)
    start_ap(AP_SSID, AP_PASSWORD, DEFAULT_AP_CHANNEL, DEFAULT_AP_BANDWIDTH);
#else
